...
const TimerZone = React.memo(function TimerZone({ elapsed, running, onStart, onPause, onNext, onPrev, onFinish, block, wd, blockTimes, currentIdx }) {
  ...
  const btn = (label, onClick, bg = "var(--bg2)", fg = "var(--tx3)", disabled = false, title = "", ariaLabel = "") => (
    <button onClick={onClick} disabled={disabled} title={title} aria-label={ariaLabel || title || label} style={{ background: bg, color: fg, border: bg === "var(--bg2)" ? "1px solid var(--bg4)" : "none", borderRadius: "var(--r-sm)", padding: "5px 10px", fontSize: 13, fontWeight: 600, cursor: disabled ? "default" : "pointer", fontFamily: "var(--sans)", lineHeight: 1, opacity: disabled ? 0.25 : 1, transition: "all .2s" }}>
      {label}
    </button>
  );
  ...
  return (
    <div style={{ background: "var(--bg2)", borderBottom: "1px solid var(--bg4)", boxShadow: "0 2px 12px rgba(0,0,0,.06)", padding: "12px 20px", position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", maxWidth: 920, margin: "0 auto" }}>
        <span aria-label={`${mm} minutos ${ss} segundos`} style={{ fontFamily: "var(--mono)", fontSize: 28, fontWeight: 700, color: overtime ? "var(--red)" : "var(--tx)", letterSpacing: "-0.05em", lineHeight: 1 }}>
          {String(mm).padStart(2, "0")}<span aria-hidden="true" style={{ animation: running ? "pulse 1s ease infinite" : "none", color: overtime ? "var(--red)" : "var(--tx3)" }}>:</span>{String(ss).padStart(2, "0")}
        </span>
        <div style={{ display: "flex", gap: 3 }}>
          {btn("⏮", onPrev, undefined, undefined, !prevB, prevB ? `← ${prevB.label}` : "", "Bloque anterior")}
          {running ? btn("⏸", onPause, "var(--yellow)", "#fff", false, "", "Pausar timer") : btn("▶", onStart, "var(--green)", "#fff", false, "", "Iniciar timer")}
          {btn("⏭", onNext, undefined, undefined, !nextB, nextB ? `${nextB.label} →` : "", "Siguiente bloque")}
          {btn("⏹", onFinish, "var(--red)", "#fff", false, "", "Finalizar weekly")}
        </div>
        <div aria-live="polite" aria-atomic="true" style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, background: urgent ? "rgba(255,69,58,.1)" : "var(--bg)", border: `0.3px solid ${urgent ? "rgba(255,69,58,.3)" : "var(--border)"}`, borderRadius: "var(--r-sm)", padding: "7px 14px", minWidth: 180, animation: urgent ? "pulse 1s ease infinite" : "none" }}>
          {running && <div aria-hidden="true" style={{ width: 6, height: 6, borderRadius: "50%", background: urgent ? "var(--red)" : block.color, animation: "liveDot 1.2s ease infinite", flexShrink: 0 }} />}
