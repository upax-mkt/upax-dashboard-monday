          ...
          <Card style={{ borderTop: "3px solid var(--purple)", marginTop: 10 }}>
            ...
            {entries.length > 0 && (
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px dashed var(--bg4)" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--tx3)", marginBottom: 6 }}>Items cross-squad directos</div>
                {entries.map((entry, idx) => (
                  <div key={idx} style={{ padding: "8px 10px", marginBottom: 4, borderRadius: 8, background: "var(--bg)", border: "1px solid var(--bg4)" }}>
                    {entry.blocker?.trim() && <div style={{ fontSize: 12, color: "var(--red)", marginBottom: 1 }}>🚫 {entry.blocker}{entry.blocker_quien ? ` → ${shortName(entry.blocker_quien)}` : ""}{entry.blocker_cuando ? ` · ${new Date(entry.blocker_cuando + "T12:00:00").toLocaleDateString("es-MX", { day: "numeric", month: "short" })}` : ""}</div>}
                    {entry.necesito?.trim() && <div style={{ fontSize: 12, color: "var(--yellow)", marginBottom: 1 }}>🤝 {entry.necesito}{entry.necesito_quien ? ` → ${shortName(entry.necesito_quien)}` : ""}{entry.necesito_cuando ? ` · ${new Date(entry.necesito_cuando + "T12:00:00").toLocaleDateString("es-MX", { day: "numeric", month: "short" })}` : ""}</div>}
