...
// Extraer status del item - busca columna tipo 'color' o 'status'
// excluye columnas de persona para no confundirlas con status
function getItemStatus(item) {
        if (!item?.column_values) return null
        // Buscar la primera columna de tipo 'color' o 'status' que no sea de persona
    const col = item.column_values.find(c =>
                (c.column?.type === 'color' || c.column?.type === 'status') &&
