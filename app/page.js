...
// Extraer status de un item de forma robusta
// Monday puede devolver type='color', 'status', o variantes
function getItemStatus(item) {
    if (!item?.column_values) return null
    // Buscar por tipo de columna: 'color' o 'status'
  const col = item.column_values.find(c =>
        c.column?.type === 'color' ||
        c.column?.type === 'status' ||
        c.column?.type === 'multiple-person' === false && (
                c.column?.title?.toLowerCase().includes('estado') ||
                c.column?.title?.toLowerCase().includes('status')
              )
                                        )
    // Si encontramos la columna, devolver el texto
  if (col?.text) return col.text
    // Fallback: buscar en el value JSON
  if (col?.value) {
        try {
...
// Extraer persona asignada
function getItemPerson(item) {
    if (!item?.column_values) return null
    const col = item.column_values.find(c =>
          c.column?.type === 'multiple-person' ||
          c.column?.type === 'person'
                                          )
...
function CargaView({ items }) {
    const personLoad = {}
        ...
        items.forEach(item => {
              const personCol = item.column_values?.find(c =>
                      c.column?.type === 'multiple-person' || c.column?.type === 'person'
                                                             )
              const status = getItemStatus(item)
  ...
  const sorted = Object.entries(personLoad).sort((a, b) => b[1].total - a[1].total)
    const totalDone = items.filter(i => getItemStatus(i) === 'Done').length
    const totalStuck = items.filter(i => getItemStatus(i) === 'Stuck').length
...
function SquadsView({ items, groups }) {
    ...
    {groups?.map(group => {
              ...
              const done = groupItems.filter(i => getItemStatus(i) === 'Done').length
              return (
...
function HealthView({ items }) {
    const statuses = {}
        items.forEach(item => {
              const status = getItemStatus(item) || 'Sin estado'
              statuses[status] = (statuses[status] || 0) + 1
        })

  const noAssignee = items.filter(i => !getItemPerson(i)).length
